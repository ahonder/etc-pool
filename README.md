...
Donations
ETC: 0x841Dc834eC4c8925b37d4eFaDCCeFca25582BD8a

### WARNING: This code is currently configured for the ETCHASH main network

### Features

### Email: ahonder@hotmail.com
### Youtube: https://www.youtube.com/channel/UCYftABQe1jUjy9crh5WZRNA
...
Install Ethereum Classic node
     ...
     Download Geth from here:(https://github.com/etclabscore/core-geth/releases)
     Unzip (unzip) or Untar (tar -xvzf) the archive
     chmod +x geth
     cp geth /usr/local/bin
     
Geth binary file must be in /usr/local/bin

After that;

     geth --classic account new

Put the password and repeat.

For automatic system service;
   
    sudo nano /etc/systemd/system/ethereumclassic.service

    [Unit]
    Description=Ethereum Classic for Pool
    After=network-online.target

    [Service]
    ExecStart=/home/ahmet/core-geth/build/bin/geth --cache=4096 --classic --http --unlock 0 --password /home/ahmet/Documents/pwd.txt --mine --http.api      eth,net,web3,txpool,miner --htt--http.port "8545" --nat "any" --port 30305 --miner.extradata ETCTurk --allow-insecure-unlock
    User=ahmet
...
CTRL+O Enter, CTRL+X
    ...
    sudo systemctl enable ethereumclassic
    sudo systemctl start ethereumclassic
...
For api,unlocker and payout services you can use same ways(need change json file)

### Building Frontend

Before all requirements must be installed
